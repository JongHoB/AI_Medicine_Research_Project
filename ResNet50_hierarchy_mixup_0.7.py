...
if __name__ == '__main__':
        ...
        def __init__(self, csv_file, dir, transform=None):
            ...
            for j in range(len(df)):
                for i in range(5, 19):
                    if i == 12 or i == 17:   # 
                        if df.iloc[j, i] == 0:
                            df.iloc[j, i] = 0.5 # 0.5로 변경
                    if df.iloc[j, i] < 0:
                        df.iloc[j, i] = 0.5    # -1 - 0.7으로 간주
    ...
    def mixup_data(x, y, alpha=1.0):
        ...
        if alpha > 0:
            lam = np.random.beta(alpha, alpha)  # beta distribution - alpha is 1 so it is same as uniform distribution
        else:
            lam = 1
        batch_size = x.size()[0]
    ...
    def train_model(model, train_loader, epoch, optimizer, criterion, scheduler, device, BATCH_SIZE,mixup_labels=[7, 12], mixup_alpha=1.0):
        ...
        with open('ResNet50_hierarchy_0.7_Train_Log.txt', 'a') as f:
            print('Epoch: {} Total Items: {} Training Loss: {:.4f} Accuracy: {:.4f} Time:{:.2f}s'.format(
    ...
    def train_model_mixup(model, train_loader, epoch, optimizer, criterion, scheduler, device, BATCH_SIZE,alpha=1.0):
        ...
        for data, labels in train_progress_bar:
            ...
            d, targets_a, targets_b, lam = mixup_data(data, labels, alpha)

            mask = (labels[:,[1,2,6,7,8,12]]> 0.7).any(dim=1)
            if mask.sum()> 1:
                data_filtered ,labels_filtered = d[mask], labels[mask]
                d_filtered, targets_a_filtered, targets_b_filtered, lam_filtered = mixup_data(data_filtered, labels_filtered, alpha)
                d[mask] = d_filtered
                targets_a[mask] = targets_a_filtered
                targets_b[mask] = targets_b_filtered

            optimizer.zero_grad()
            
            d, targets_a, targets_b = d.to(device), targets_a.to(device), targets_b.to(device)
            result_hat = model(d)
            ...
            # labels 와 비교
            total_correct += ((result_hat > 0.7) == targets_a).sum().item()
    ...
    def valid_model(model, val_loader, epoch, criterion, device, batch_size, scheduler):
        ...
        with open('ResNet50_hierarchy_0.7_Valid_Log.txt', 'a') as f:
            print('Valid Epoch: {} Total Items {} Validation Loss: {:.4f} Accuracy: {:.2f}%'.format(
    ...
    def loss_plot(train_loss_list, valid_loss_list):
        ...
        # save the plot
        plt.savefig('ResNet50_hierarchy_0.7_loss_plot.png')
    ...
    def acc_plot(train_acc_list, valid_acc_list):
        ...
        # save the plot
        plt.savefig('ResNet50_hierarchy_0.7_acc_plot.png')
    ...
    def test_model(model, test_loader, criterion, device):
        ...
        with open('ResNet50_hierarchy_0.7_Test_Log.txt', 'a') as f:
            print(scores, file=f)
        ...
        for i in range(14):
            ...
            plt.savefig('ResNet50_hierarchy_0.7_ROC_Curve_Label_{}.png'.format(cols[i]))
    ...
    def scores_plot(test_scores_list, epoch):
        for i in range(14):
            ...
            plt.legend()
            plt.savefig('ResNet50_hierarchy_0.7_Scores_Label_{}.png'.format(cols[i]))
    ...
    for t in range(300):
        ...
        torch.save(model.state_dict(), base_path+'ResNet50_hierarchy_0.7.pth')
        ...
        if loss < best_loss:
            best_loss = loss
            best_model = copy.deepcopy(model.state_dict())
            torch.save(best_model, base_path+'ResNet50_hierarchy_0.7_best.pth')
