...
if __name__ == '__main__':
    ...
    def train_model(model, train_loader, epoch, optimizer, criterion, scheduler, device, BATCH_SIZE):
        model.train()
        ...
        for data, labels in train_progress_bar:
            ...
            # Multi label 각 label이 0.6 이상이면 1로 간주, 0.6 미만이면 0으로 간주
            # labels 와 비교
        ...
        with open('EfficientNet_train_log.txt', 'a') as f:
            print('Epoch: {} Total Items: {} Training Loss: {:.4f} Accuracy: {:.4f} Time:{:.2f}s'.format(
    ...
    def valid_model(model, val_loader, epoch, criterion, device, batch_size, scheduler):
        ...
        # save the print to a file
        with open('EfficientNet_valid_log.txt', 'a') as f:
            print('Valid Epoch: {} Total Items {} Validation Loss: {:.4f} Accuracy: {:.2f}%'.format(
    ...
    def loss_plot(train_loss_list, valid_loss_list):
        ...
        # save the plot
        plt.savefig('EfficientNet_loss_plot.png')
    ...
    def acc_plot(train_acc_list, valid_acc_list):
        ...
        # save the plot
        plt.savefig('EfficientNet_acc_plot.png')
    ...
    def test_model(model, test_loader, criterion, device):
        ...
        # save the print to a file
        # sys.stdout = open('EfficientNet_test_log.txt', 'a')
        # print(scores)
        # sys.stdout.close()
        with open('EfficientNet_test_log.txt', 'a') as f:
            print(scores, file=f)
        ...
        for i in range(14):
            ...
            plt.savefig(
                'EfficientNet_ROC_Curve_Label_{}.png'.format(cols[i]))
    ...
    def scores_plot(test_scores_list, epoch):
        for i in range(14):
            ...
            plt.legend()
            plt.savefig('EfficientNet_Scores_Label_{}.png'.format(cols[i]))
    ...
    for t in range(300):
        ...
        torch.save(model.state_dict(), base_path+'EfficientNet.pth')
        ...
        if loss < best_loss:
            best_loss = loss
            best_model = copy.deepcopy(model.state_dict())
            torch.save(best_model, base_path+'EfficientNet_best.pth')
