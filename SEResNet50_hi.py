...
if __name__ == '__main__':
    ...
    def train_model(model, train_loader, epoch, optimizer, criterion, scheduler, device, BATCH_SIZE,mixup_labels=[7, 12], mixup_alpha=1.0):
        ...
        with open('SEResNet50_hierarchy_non_Train_Log.txt', 'a') as f:
            print('Epoch: {} Total Items: {} Training Loss: {:.4f} Accuracy: {:.4f} Time:{:.2f}s'.format(
    ...
    def valid_model(model, val_loader, epoch, criterion, device, batch_size, scheduler):
        ...
        with open('SEResNet50_hierarchy_non_Valid_Log.txt', 'a') as f:
            print('Valid Epoch: {} Total Items {} Validation Loss: {:.4f} Accuracy: {:.2f}%'.format(
    ...
    def loss_plot(train_loss_list, valid_loss_list):
        ...
        # save the plot
        plt.savefig('SEResNet50_hierarchy_non_loss_plot.png')
    ...
    def acc_plot(train_acc_list, valid_acc_list):
        ...
        # save the plot
        plt.savefig('SEResNet50_hierarchy_non_acc_plot.png')
    ...
    def test_model(model, test_loader, criterion, device):
        ...
        with open('SEResNet50_hierarchy_non_Test_Log.txt', 'a') as f:
            print(scores, file=f)
        ...
        for i in range(14):
            ...
            plt.savefig('SEResNet50_hierarchy_non_ROC_Curve_Label_{}.png'.format(cols[i]))
    ...
    def scores_plot(test_scores_list, epoch):
        for i in range(14):
            ...
            plt.legend()
            plt.savefig('SEResNet50_hierarchy_non_Scores_Label_{}.png'.format(cols[i]))
    ...
    for t in range(300):
        ...
        torch.save(model.state_dict(), base_path+'SEResNet50_hierarchy_non.pth')
        ...
        if loss < best_loss:
            best_loss = loss
            best_model = copy.deepcopy(model.state_dict())
            torch.save(best_model, base_path+'SEResNet50_hierarchy_non_best.pth')
